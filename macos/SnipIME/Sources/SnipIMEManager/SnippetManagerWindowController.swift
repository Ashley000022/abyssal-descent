...
final class SnippetManagerWindowController: NSWindowController, NSTableViewDataSource,
  NSTableViewDelegate, NSSearchFieldDelegate, NSTextViewDelegate, NSWindowDelegate
{
  private let store: SnippetStore
  private var snippets: [Snippet] = []
  private var filteredSnippets: [Snippet] = []
  private var selectedID: UUID?
  private var editorIsDirty = false
  ...
  func tableViewSelectionDidChange(_ notification: Notification) {
    guard tableView.selectedRow >= 0, filteredSnippets.indices.contains(tableView.selectedRow)
    else {
      selectedID = nil
      updateEditor(nil)
      return
  ...
  func selectionShouldChange(in tableView: NSTableView) -> Bool {
    confirmDiscardingChangesIfNeeded()
  }

  func controlTextDidChange(_ obj: Notification) {
    if let field = obj.object as? NSSearchField, field === searchField {
      guard !editorIsDirty else {
        showStatus("先に変更を保存してください", isError: true)
        return
      }
      applyFilter(selecting: selectedID)
    } else if !isUpdatingEditor {
      editorIsDirty = true
    }
  }

  func textDidChange(_ notification: Notification) {
    if !isUpdatingEditor { editorIsDirty = true }
  }
  ...
  @objc private func addSnippet() {
    ...
    var suffix = 1
    var shortcut = "new"
    while snippets.contains(where: { $0.shortcut == shortcut }) {
      suffix += 1
      shortcut = "new\(suffix)"
    }
    let snippet = Snippet(
  ...
  private func persistEditor() -> Bool {
    ...
    do {
      let updated = try store.updateEditableFields(
        id: selectedID,
        title: titleField.stringValue,
        shortcut: shortcut,
        content: contentView.string
      )
      editorIsDirty = false
  ...
  private func buildInterface() {
    ...
    let detail = NSView()
    let divider = NSBox()
    for view in [sidebar, detail, divider] {
  ...
  private func buildDetail(in container: NSView) {
    ...
    let settingsButton = NSButton(
      title: "キーボード設定を開く", target: self, action: #selector(openKeyboardSettings))
    ...
    for view in [helper, titleField, shortcutField, contentScroll, usageLabel] {
  ...
  private func applyFilter(selecting id: UUID?) {
    ...
    let desired =
      id.flatMap { wanted in filteredSnippets.firstIndex(where: { $0.id == wanted }) }
  ...
  private func updateEditor(_ snippet: Snippet?) {
    ...
    for control in [titleField, shortcutField, saveButton, deleteButton] {
  ...
  private func confirmDiscardingChangesIfNeeded() -> Bool {
    guard editorIsDirty else { return true }

    let alert = NSAlert()
    alert.messageText = "変更を保存しますか？"
    alert.informativeText = "保存しない場合、編集中の変更は失われます。"
