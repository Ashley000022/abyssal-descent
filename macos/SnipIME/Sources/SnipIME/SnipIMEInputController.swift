...
final class SnipIMEInputController: IMKInputController {
  ...
  override func candidateSelected(_ candidateString: NSAttributedString!) {
    guard
      ...
      engine.candidates.indices.contains(index)
    else { return }

    _ = render(engine.selectCandidate(id: engine.candidates[index].id), client: client)
  }
  ...
  override func candidateSelectionChanged(_ candidateString: NSAttributedString!) {
    ...
    while engine.selection != index {
      let distanceForward =
        (index - engine.selection + engine.candidates.count) % engine.candidates.count
      let distanceBackward =
        (engine.selection - index + engine.candidates.count) % engine.candidates.count
  ...
  override func inputControllerWillClose() {
    if engine.isComposing, let activeClient {
      _ = render(engine.commitLiteral(), client: activeClient)
    }
    SnipIMEAppDelegate.candidateWindow?.hide()
    activeClient = nil
  ...
  private func render(_ action: SnippetEngineAction, client: IMKTextInput) -> Bool {
    ...
    switch action {
    case .passThrough:
      // Let the host application handle ordinary text and key bindings.
      return false
  ...
  private func selectedCandidateIndex() -> Int? {
    guard let window = SnipIMEAppDelegate.candidateWindow else { return nil }
    let index = window.selectedCandidate()
